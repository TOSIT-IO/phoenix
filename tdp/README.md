...
```
mvn clean install -DskipTests -P phoenix-hbase-compat-2.6.0
```

This command generates `phoenix-hbase-2.6-5.2.1-0.0-bin.tar.gz` file in the `phoenix-assembly/target` directory.
...
```
mvn verify -DPhoenixPatchProcess -Dskip.code-coverage -P phoenix-hbase-compat-2.6.0
```
